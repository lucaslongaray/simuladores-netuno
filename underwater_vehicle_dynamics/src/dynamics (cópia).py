...
from underwater_vehicle_dynamics.srv import waterCurrentService
...
#       rospy.loginfo("s(): \n %s", x)
    ...
    def pubWaterCurrentForce(self, event):
        p = Point()
        p.x = self.current_mean[0]
        p.y = self.current_mean[1]
        p.z = self.current_mean[2]
        self.pubCurrent.publish(p)
	
    def setWaterCurrentService(self, req):
        self.current_mean[0]=req.x
        self.current_mean[1]=req.y
        self.current_mean[2]=req.z
        return 1
    ...
    def inverseDynamic(self) :
        ...
        waterCurrentForce = [self.current_mean[0]*cos(-self.p[5]) - self.current_mean[1]*sin(-self.p[5]),
                             self.current_mean[0]*sin(-self.p[5]) + self.current_mean[1]*cos(-self.p[5]),
		  	     -self.current_mean[2],
			     0,
 	  	 	     0,
                             0]

        v_dot = dot(self.IM, (t-c_v-g+self.collisionForce + waterCurrentForce)) #t-c_v-g+collisionForce+waterCurrentForce
...
#
	...
	#pose.position.x = event.pose.position.x
        #pose.position.y = event.pose.position.y
        #pose.position.z = -event.pose.position.z
        #pose.orientation.x = event.pose.orientation.x
        #pose.orientation.y = event.pose.orientation.y
        #pose.orientation.z = event.pose.orientation.z
        #pose.orientation.w = event.pose.orientation.w
	
        #orientation = (event.pose.orientation.x, event.pose.orientation.y, event.pose.orientation.z, event.pose.orientation.w)
        ...
        br.sendTransform((event.pose.position.x, event.pose.position.y, event.pose.position.z), orientation, 
        rospy.Time.now(), "world", str(self.frame_id))
    
    def pubVelocityOrientation(self, event):
        data = Twist()

        data.linear.x = self.v[0]
        data.linear.y = self.v[1]
        data.linear.z = self.v[2]
        data.angular.x = self.p[3]
        data.angular.y = self.p[4]
        data.angular.z = self.p[5]

        self.pub_velocityorientation.publish(data)
	...
	self.pub_velocityorientation = rospy.Publisher('/g500/velocity_and_Orientation', Twist, queue_size=10)
	...
	self.pubCurrent = rospy.Publisher('waterCurrentForce', Point, queue_size=10) 
	#The number of zeros will depend on the number of actuators
        ...
        rospy.Timer(rospy.Duration(self.uwsim_period), self.pubVelocityOrientation)
        rospy.Timer(rospy.Duration(self.uwsim_period), self.pubWaterCurrentForce)
	...
	#TODO: set the topic names as parameters
        ...
        rospy.Subscriber('/rexrov/ground_truth_to_tf/pose', PoseStamped, self.pubPose)

        t = rospy.Service('/dynamics/setWaterCurrentForce', waterCurrentService, self.setWaterCurrentService)
